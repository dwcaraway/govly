...
class FBOTask:
    ...
    def extract(self):
        ...
        """
        self.sync_daily_files()
        self.sync_weekly_file()

    def sync_daily_files(self):
        conn = S3Connection()
        ...
        try:
            ...
            self.sourceModifiedDateTime = datetime.strptime(sourceModifiedTime, "%Y%m%d%H%M%S")

            delta = self.sourceModifiedDateTime - datetime.today()
            daily_files = []
            ...
            for f in daily_files:
                local_file_path = path.join(self.temp_dir, f)

                #See if key already exists in S3, if so, move on
                k = vitals_bucket.get_key(S3_EXTRACT_PREFIX+f+S3_ARCHIVE_FORMAT)
                if k:
                    continue
                ...
                #Compress the file
                zipped_storage_path = path.join(self.temp_dir, f+S3_ARCHIVE_FORMAT)
                with gzip.GzipFile(zipped_storage_path, 'w') as myzip:
    ...
    def sync_weekly_file(self):
        ...
        try:
            if not fullFBOKey or parse_ts(fullFBOKey.last_modified) < self.sourceModifiedDateTime:
                #Update S3 copy with latest

                print "downloading the latest full xml from repository"
                storage_path = path.join(self.temp_dir, 'FBOFullXML.xml')
                ...
                zipped_storage_path = path.join(self.temp_dir, 'FBOFullXML'+S3_ARCHIVE_FORMAT)
                with gzip.GzipFile(zipped_storage_path, 'w') as myzip:
                ...
                mp = vitals_bucket.initiate_multipart_upload(S3_EXTRACT_PREFIX+'FBOFullXML'+S3_ARCHIVE_FORMAT)
